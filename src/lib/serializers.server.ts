import type {
  ...
  AdminCustomerView as PrismaAdminCustomerView,
} from "@prisma/client";
import type {
  Account,
  Transaction,
...
export function serializeAdminCustomer(c: PrismaAdminCustomerView): AdminCustomer {
  return {
    id: c.id,
    name: c.name,
    email: c.email,
    status: c.status,
    kyc: c.kyc,
    joined: c.joined.toISOString().slice(0, 10),
