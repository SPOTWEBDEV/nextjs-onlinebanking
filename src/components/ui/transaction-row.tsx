...
export function TransactionRow({ transaction, onClick }: { transaction: Transaction; onClick?: () => void }) {
  ...
  const displayAmount = transaction.currency === "EUR" ? convert(transaction.amount) : transaction.amount;
  const currencyLabel = transaction.currency === "EUR" ? displayCurrency : transaction.currency;
