...
const quickActions = [
  ...
  { href: "/dashboard/deposit", label: "Deposit", icon: WalletCards },
    ...
    <div>
      ...
      <div className="px-5 pb-4 pt-2">
        <p className="text-sm text-muted-foreground">
          {greetingForTime()}, {user.fullName.split(" ")[0]} 👋
        </p>
      </div>
      ...
      <div className="px-5">
        {loadingAccounts || !primary ? (
          ...
          <VaultCard label={primary.nickname} amount={convert(primary.balance)} currency={displayCurrency} />
        )}
        <div className="mt-3 flex items-center justify-between rounded-2xl bg-muted px-4 py-2.5 text-xs">
          ...
          <span className="font-mono font-semibold tabular">{formatCurrency(convert(totalBalance), displayCurrency)}</span>
        </div>
      </div>
