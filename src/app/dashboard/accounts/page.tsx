...
function AccountCard({ account }: { account: Account }) {
  ...
  const isConverted = account.currency === "EUR" && displayCurrency !== "EUR";

  return (
    <Card className="overflow-hidden">
      <VaultCard
        label={account.nickname}
        amount={isConverted ? convert(account.balance) : account.balance}
        currency={isConverted ? displayCurrency : account.currency}
      ...
      <div className="p-4">
        ...
        <div className="grid grid-cols-2 gap-x-3 gap-y-1 border-t border-border pt-2">
          ...
          <DetailRow label="Available balance" value={formatCurrency(isConverted ? convert(account.availableBalance) : account.availableBalance, isConverted ? displayCurrency : account.currency)} />
          <DetailRow label="Ledger balance" value={formatCurrency(isConverted ? convert(account.ledgerBalance) : account.ledgerBalance, isConverted ? displayCurrency : account.currency)} />
        </div>
      </div>
    </Card>
  );
