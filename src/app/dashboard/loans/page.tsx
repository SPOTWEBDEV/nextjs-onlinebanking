...
function EmiCalculator({ product, onApply }: { product: LoanProduct; onApply: (amount: number, term: number) => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <Button size="lg" className="w-full" onClick={() => onApply(amount, term)}>
...
export default function LoansPage() {
  ...
  const { data: loans } = useQuery({ queryKey: ["loans"], queryFn: fetchLoans });
